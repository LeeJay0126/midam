
const DrinkTypeList = [
    ["BEER", [
        ["budbeer.webp", "Domestic Beer", "$5", "Domestic Beer"],
        ["asahi.jpeg", "Imported Beer", "$6", "Imported Beer"],
        ["sapporo.webp", "Imported Large", "$11", "Imported Large"],
    ]],
    ["KOREAN LIQUOR", [
        ["soju.jpg", "Soju", "$17", "Soju, 17%"],
        ["peachsoju.jpeg", "Peach Flavour Soju", "$17", "Peach Flavour Soju, 13%"],
        ["applesoju.jpg", "Apple Flavour Soju", "$17", "Apple Flavour Soju, 13%"],
        ["greenapplesoju.jpg", "Green Grape Flavour Soju", "$17", "Green Grape Flavour Soju, 13%"],
        ["strawberrysoju.webp", "Strawberry Flavour Soju", "$17", "Strawberry Flavour Soju, 13%"],
        ["yogurtsoju.webp", "Yogurt Flavour Soju", "$17", "Yogurt Flavour Soju, 13%"],
        ["plumwine.webp", "Suljoongme", "$21", "Plum Wine, 14%"],
        ["blackberrywine.webp", "Bokbunja", "$18", "Blackberry Wine, 12%"],
    ]],
    ["RICE WINE", [
        ["originalmgl.jpg", "Original Ricewine", "$11", "Original Rice Wine, 6.5%"],
        ["chestnutmgl.jpg", "Chestnut Ricewine", "$11", "Chestnut Rice Wine, 6.5%"],
        ["question.jpeg", "Citron Ricewine", "$11", "Citron Rice Wine, 6.5%"],
        ["question.jpeg", "Green Grape Ricewine", "$11", "Green Grape Rice Wine, 6.5%"],
        ["strawberrymgl.jpg", "Strawberry Ricewine", "$15", "Strawberry Rice Wine, 6%"],
    ], true, "locally brewed"],
    ["BEVERAGE", [
        ["milkis.webp", "Milkis", "$3.5", "Milkis"],
        ["bongbong.webp", "Grape Bongbong", "$4", "Grape Bongbong"],
        ["idh.webp", "Pear Juice", "$4", "Pear Juice"],
        ["coke.webp", "Coke", "$3", "Coke"],
        ["dietcoke.webp", "Diet Coke", "$3", "Diet Coke"],
        ["7up.webp", "7up", "$3", "7up"],
        ["nestea.webp", "Iced Tea", "$3", "Iced Tea"],
        ["gingerale.webp", "Ginger Ale", "$3", "Ginger Ale"],
    ]],
    ["SHAKE & SMOOTHIE", [
        ["roastedgrain.webp", "21 Grain Shake", "$7", "Roasted Grain Shake"],
        ["question.jpeg", "Gelato Milk Shake", "$7", "Chocolate Gelato Milkshake"],
        ["question.jpeg", "Gelato Milk Shake", "$7", "Green Tea Gelato Milkshake"],
        ["question.jpeg", "Gelato Milk Shake", "$7", "Vanilla Gelato Milkshake"],
        ["question.jpeg", "Gelato Milk Shake", "$7", "Oreo Gelato Milkshake"],
        ["question.jpeg", "Gelato Milk Shake", "$7", "Strawberry Gelato Milkshake"],
        ["question.jpeg", "Real Fruit Yogurt Smoothie", "$8", "Real Fruit Yogurt Smoothie (Blueberry)"],
        ["question.jpeg", "Real Fruit Yogurt Smoothie", "$8", "Real Fruit Yogurt Smoothie (Strawberry)"],
    ]],
];

export default DrinkTypeList;